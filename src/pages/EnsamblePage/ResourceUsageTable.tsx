import { Tooltip, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, Avatar, styled, type TableRowProps, Container } from '@mui/material';
...
import { type MeasureQuality, type Runner, type ServiceStatus } from '@/services/healthCheckService';
...
const ResourceUsageTable: React.FC<IResourceUsageTableProps> = ({ data }) => (
    <TableContainer component={Container}>
        <Table sx={{ minWidth: 650 }}>
            ...
            <TableBody>
            {data.map((runner) => (
                <StyledTableRow key={runner.name} dimmed={!["GOOD", "ACCEPTABLE"].includes(runner.quality)}>
                <TableCell>
                    <Tooltip title={runner.status}>
                    <Avatar sx={{ bgcolor: statusColors[runner.status] }}>
                ...
                <TableCell>
                  <ServiceStatusChip status={runner?.status} quality={runner?.quality} />
                </TableCell>
                <TableCell>
                    {qualityLabel[runner.quality]}
                </TableCell>
                <TableCell align="right">
                    {runner.resources.cpuUtilization <= 1 ? '<1' : runner.resources.cpuUtilization}
                </TableCell>
                <TableCell align="right">{runner.resources.memoryUsage}</TableCell>
                <TableCell align="right">{runner.resources.peakMemoryUsage}</TableCell>
                <TableCell align="right">{runner.resources.threadCount}</TableCell>
                <TableCell align="right">{runner.resources.handleCount}</TableCell>
                </StyledTableRow>
            ))}
            </TableBody>
        </Table>
